...
const AddIndividual = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.firstName || !formData.lastName || !formData.primaryPhone || !formData.sex || !deliveryDate) {
    ...
    newIndividualData.append('phoneNumbers[secondary]', formData.secondaryPhone);
    ...
    if (deliveryDate) {
      newIndividualData.append('deliveryDate', deliveryDate.toISOString().split('T')[0]);
    }

    // Cloth Details
    newIndividualData.append('clothDetails[colors]', formData.colorCodes.split(',').map(code => code.trim()).filter(code => code !== '').join(','));
    if (tilefFile) {
    ...
    if (formData.sex === 'Female') {
      newIndividualData.append('clothDetails[dressLength]', formData.dressLength);
      newIndividualData.append('clothDetails[sliveLength]', formData.sliveLength);
      ...
      newIndividualData.append('clothDetails[underBreast]', formData.underBreast);
      newIndividualData.append('clothDetails[femaleSliveType]', formData.femaleSliveType);
      newIndividualData.append('clothDetails[femaleWegebType]', formData.femaleWegebType);
    }
    if (formData.sex === 'Male') {
      newIndividualData.append('clothDetails[deret]', formData.deret);
      newIndividualData.append('clothDetails[anget]', formData.anget);
      newIndividualData.append('clothDetails[maleClothType]', formData.maleClothType);
      newIndividualData.append('clothDetails[maleSliveType]', formData.maleSliveType);
      newIndividualData.append('clothDetails[netela]', formData.netela);
    }

    // Payment Details
    newIndividualData.append('payment[total]', formData.total);
    newIndividualData.append('payment[firstHalf][paid]', String(formData.firstHalfPaid));
    newIndividualData.append('payment[firstHalf][amount]', formData.firstHalfAmount);
    newIndividualData.append('payment[secondHalf][paid]', String(formData.secondHalfPaid));
    newIndividualData.append('payment[secondHalf][amount]', formData.secondHalfAmount);
    
    try {
      await addIndividual(newIndividualData);
      toast({ title: "Success", description: "Individual order created successfully" });
      navigate("/orders");
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-serif font-bold text-primary mb-2">Add Individual Order</h1>
        <p className="text-muted-foreground">Create a new individual cloth order</p>
      </div>
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end"><Button type="submit" className="bg-gradient-primary text-secondary-foreground shadow-elegant hover:shadow-xl transition-all px-8 py-3">Save Individual Order</Button></div>
      </form>
    </div>
  );
};
