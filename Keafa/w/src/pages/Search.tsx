            ...
            {selectedOrder?.type === 'family' && (() => {
              ...
              return (
                <><DialogHeader><DialogTitle>{order.familyName} - Details</DialogTitle></DialogHeader><div className="space-y-6 py-4">
                  ...
                  <Card><CardHeader><DialogTitle className="text-lg">Payment</DialogTitle></CardHeader><CardContent><DetailRow label="Total" value={order.payment?.total ? `ETB ${order.payment.total.toLocaleString()}` : 'N/A'} /><DetailRow label="First Half Status" value={order.payment?.firstHalf.paid ? 'Paid' : 'Pending'} /><DetailRow label="First Half Amount" value={order.payment?.firstHalf.amount ? `ETB ${order.payment.firstHalf.amount.toLocaleString()}` : 'N/A'} /><DetailRow label="Second Half Status" value={order.payment?.secondHalf.paid ? 'Paid' : 'Pending'} /><DetailRow label="Second Half Amount" value={order.payment?.secondHalf.amount ? `ETB ${order.payment.secondHalf.amount.toLocaleString()}` : 'N/A'} /></CardContent></Card>
