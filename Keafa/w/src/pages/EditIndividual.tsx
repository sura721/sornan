...
import { CalendarIcon, Upload, X, ArrowUpRight, Loader2 } from "lucide-react";
...
// Find and replace your entire handleSubmit function with this one.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updatedData.append('_id', id);
    updatedData.append('firstName', formData.firstName);
    updatedData.append('lastName', formData.lastName);
    updatedData.append('phoneNumbers[primary]', formData.primaryPhone);
    updatedData.append('phoneNumbers[secondary]', formData.secondaryPhone);
    updatedData.append('socials[telegram]', formData.telegramUsername);
    updatedData.append('socials[instagram]', formData.instagramUsername);
    updatedData.append('sex', formData.sex);
    updatedData.append('age', formData.age);
    updatedData.append('clothDetails[colors]', formData.colorCodes);
    updatedData.append('clothDetails[shirtLength]', formData.shirtLength);
    ...
    if (formData.sex === 'Female') {
      ...
      updatedData.append('clothDetails[sliveLength]', formData.sliveLength);
      updatedData.append('clothDetails[breast]', formData.breast);
      updatedData.append('clothDetails[overBreast]', formData.overBreast);
      updatedData.append('clothDetails[underBreast]', formData.underBreast);
      updatedData.append('clothDetails[femaleSliveType]', formData.femaleSliveType);
      updatedData.append('clothDetails[femaleWegebType]', formData.femaleWegebType);
    } else {
      updatedData.append('clothDetails[deret]', formData.deret);
      updatedData.append('clothDetails[anget]', formData.anget);
      updatedData.append('clothDetails[maleClothType]', formData.maleClothType);
      updatedData.append('clothDetails[maleSliveType]', formData.maleSliveType);
      updatedData.append('clothDetails[netela]', formData.netela);
    }

    updatedData.append('payment[total]', formData.total);
    updatedData.append('payment[firstHalf][paid]', String(formData.firstHalfPaid));
    updatedData.append('payment[firstHalf][amount]', formData.firstHalfAmount);
    updatedData.append('payment[secondHalf][paid]', String(formData.secondHalfPaid));
    updatedData.append('payment[secondHalf][amount]', formData.secondHalfAmount);

    if (deliveryDate) {
      updatedData.append('deliveryDate', deliveryDate.toISOString().split('T')[0]);
    }
    ...
    console.log("Data being sent to backend:", Object.fromEntries(updatedData.entries()));

    try {
      await updateIndividual(id, updatedData);
