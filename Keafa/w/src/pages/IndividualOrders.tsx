...
const IndividualOrders = () => {
  ...
  return (
    <div className="space-y-8 p-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            View all individual cloth orders
          </p>
        </div>
      </div>
        ...
        <Card className="shadow-card border-0 p-8 text-center">
          <div className="space-y-4">
            ...
            <h3 className="text-xl font-semibold text-primary">
              No Individual Orders Yet
            </h3>
            <p className="text-muted-foreground">
              Click 'Add New Individual' to create an order.
            </p>
          </div>
        </Card>
      ) : (
          ...
          {individuals.map((individual) => (
            ...
            >
              <CardContent
                ...
                onClick={() => setSelectedIndividualId(individual._id)}
              >
                <div className="space-y-4 flex-grow">
                  <div className="w-full h-48 bg-accent rounded-lg flex items-center justify-center overflow-hidden">
                    {individual.clothDetails?.tilefImageUrl ? (
                      <img
                        src={getImageUrl(individual.clothDetails.tilefImageUrl)}
                        alt="Tilef Design"
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {individual.phoneNumbers?.primary}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Delivery:{" "}
                ...
                <div className="mt-auto pt-4 space-y-4">
                  ...
                  >
                    <span className="font-medium text-primary text-sm">
                      ETB {(individual.payment?.total || 0).toLocaleString()}
                    </span>
                    <div className="flex gap-2">
                      <Button asChild size="sm" variant="outline">
                        <Link to={`/edit-individual/${individual._id}`}>
                  ...
                  <div className="flex justify-center gap-2 text-xs">
                    <span
                      className={`px-2 py-1 rounded-full ${
                        individual.payment?.firstHalf?.paid
                          ? "bg-green-100 text-green-700"
                          : "bg-red-100 text-red-700"
                      }`}
                    >
                      1st:{" "}
                      {individual.payment?.firstHalf?.paid ? "Paid" : "Pending"}
                    </span>
                    <span
                      className={`px-2 py-1 rounded-full ${
                        individual.payment?.secondHalf?.paid
                          ? "bg-green-100 text-green-700"
                          : "bg-red-100 text-red-700"
                      }`}
                    >
      ...
      <Dialog
        open={!!selectedIndividualId}
        onOpenChange={() => setSelectedIndividualId(null)}
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          {selectedIndividualId &&
            (() => {
              const order = individuals.find(
                (ind) => ind._id === selectedIndividualId
              );
              if (!order) return null;
              const { clothDetails, payment } = order;
              const isNumberValue = (val: unknown): val is number =>
                typeof val === "number";
              return (
                <>
                  <DialogHeader>
                    <DialogTitle>
                      {order.firstName} {order.lastName} - Order Details
                    </DialogTitle>
                  </DialogHeader>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                          {/* --- Common measurements --- */}
                          {isNumberValue(clothDetails.shirtLength) && (
                            <DetailRow
                              label="Shirt Length"
                              value={clothDetails.shirtLength}
                            />
                          )}
                          {isNumberValue(clothDetails.sholder) && (
                            <DetailRow
                              label="Sholder"
                              value={clothDetails.sholder}
                            />
                          )}
                          {isNumberValue(clothDetails.wegeb) && (
                            <DetailRow
                              label="Waist"
                              value={clothDetails.wegeb}
                            />
                          )}
                          {isNumberValue(clothDetails.rist) && (
                            <DetailRow
                              label="Wrist"
                          ...
                          {order.sex === "Female" && (
                            <>
                              {isNumberValue(clothDetails.dressLength) && (
                                <DetailRow
                                  label="Dress Length"
                                  value={clothDetails.dressLength}
                                />
                              )}
                              {isNumberValue(clothDetails.sliveLength) && (
                                <DetailRow
                                  label="Sleeve Length"
                                  value={clothDetails.sliveLength}
                                />
                              )}
                              {isNumberValue(clothDetails.breast) && (
                                <DetailRow
                                  label="Breast"
                                  value={clothDetails.breast}
                                />
                              )}
                              {isNumberValue(clothDetails.overBreast) && (
                                <DetailRow
                                  label="Over Breast"
                                  value={clothDetails.overBreast}
                                />
                              )}
                              {isNumberValue(clothDetails.underBreast) && (
                                <DetailRow
                                  label="Under Breast"
                          ...
                          {order.sex === "Male" && (
                            <>
                              {isNumberValue(clothDetails.deret) && (
                                <DetailRow
                                  label="Deret"
                                  value={clothDetails.deret}
                                />
                              )}
                              {isNumberValue(clothDetails.anget) && (
                                <DetailRow
                                  label="Neck"
